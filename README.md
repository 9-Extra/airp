...
AI一共四个工具，减少注意力浪费：
- **持久化代码环境**：内置 `world_run` 工具提供持久 JavaScript 环境（`node:vm`，按会话分键）。所有数值、随机性与规则判定由代码完成；全局对象 `state` 与全局函数自动持久化，跨会话恢复；每次执行返回 state 变化 diff 供模型确认。约定式整理：定义全局函数 `normalize()` 即会在每次成功执行后、diff 前被自动调用（变量钳制、阈值提醒写在里面）。原子执行与试运行：代码出错自动整体回滚（state/函数定义），不留半更新；`dry: true` 试运行返回完整结果但不提交任何变化，供复杂结算前排错。
